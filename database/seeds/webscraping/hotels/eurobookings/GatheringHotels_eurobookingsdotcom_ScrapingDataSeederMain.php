                ...
                $crawler->filter('div#idSearchList > table.clsHotelListAvailable > tbody > tr')->each(function ($node) {
                    ...
                    $da['all_data'] = $node->filter('.clsHotelNameSearchResults')->each(function ($node) {
                        ...
                        try {
                            ...
                            $rooms = $crawler->filter('table#idEbAvailabilityRoomsTable > tbody')->each(function ($node) {
                                ...
                                $dr['rooms_prices'] = $node->filter('tr')->each(function ($node1) {
                                    ...
                                    if ($node1->filter('.clsRoomPhotoWrap > img')->count() > 0) {
                                        $dr['room_img'] = $_SESSION['img'] = str_replace('//', '', $node1->filter('.clsRoomPhotoWrap > img')->attr('src'));
                                    }
                            ...
                            $hid = 'hotelname' . trim(str_replace(' ', '', $dh['hotel_name'])) . 'hoteladdress' . trim(str_replace(' ', '', $dh['hotel_address']));


                            if (DB::table('hotels_eurobookings')->where('hid', '=', $hid)->doesntExist()) {
                            ...
                            foreach ($dr['all_rooms'] as $room) {
                                ...
                                $rid = $rid = 'currentdate' . $requestDate . 'checkin' . $checkInDate . 'checkout' . $checkOutDate . 'hotelname' . trim(str_replace(' ', '', $dh['hotel_name'])) . 'room' . trim(str_replace(' ', '', $room['name'])) . $room['price']; //Requestdate + CheckInDate + CheckOutDate + HotelId + RoomName + number of adults
